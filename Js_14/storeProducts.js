...
let telephone = document.querySelector("#telephon"); 
...
function filterBox(){ 
    ...
    for(i = 0; i < storeProducts.length;i++){ 
        if(((storeProducts[i].name_prefix == "Ноутбук" && laptops.checked == true)|| (storeProducts[i].name_prefix == "Смартфон" && telephone.checked == true)) || (laptops.checked == false && telephone.checked == false)){ 
       if(((storeProducts[i].brend == "Dell" && dell.checked == true ) || (storeProducts[i].brend == "Apple" && Apple.checked == true) || (storeProducts[i].brend == "Samsung" && Samsung.checked == true)) || (dell.checked == false && Apple.checked == false && Samsung.checked == false)){
        if((parseInt(storeProducts[i].prices) >= minPrice.value && parseInt(storeProducts[i].prices) <= maxPrice.value) || (maxPrice.value == "" && minPrice.value== "")){  
    createCard(storeProducts[i]); 
 } } } } 
}
        
function createCard(obj) {   
    let card = document.createElement('div');   
   card.classList.add("ustroistva");  
   card.id =  "ustrostvo"; 
    ...
    header.innerText = obj.full_name;   
    header.id =  "ustrostvo"; 
   
    let desript = document.createElement('p');   
    desript.innerText = obj.description;  
    desript.id =  "ustrostvo"; 
 
    let price = document.createElement('span');   
    price.innerText = obj.prices;  
...
function deleteCards() {   
    let cards = document.querySelectorAll('#ustrostvo');   
...
for(i = 0; storeProducts.length > i; i++){  
    createCard(storeProducts[i]);  
} 
filterAfter.addEventListener("click",filterMove); 
send.addEventListener("click",filterBox);